...
const VetUploadModal = ({ projectId, onClose, onSubmitted }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-xl max-w-md w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center">
          <h3 className="font-bold text-gray-900 text-lg">Veterinar hujjati</h3>
          <button onClick={onClose} aria-label="Yopish" className="text-gray-400 hover:text-gray-600 text-lg">&times;</button>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Veterinar F.I.SH</label>
            <input
              type="text"
              value={vetName}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5">Litsenziya raqami</label>
              <input
                type="text"
                value={vetLicenseNo}
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5">Ko'rik sanasi</label>
              <input
                type="date"
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Hayvonlar holati</label>
            <select
              value={healthStatus}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Xulosa (ixtiyoriy)</label>
            <textarea
              value={conclusion}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">Hujjatlar (PDF yoki foto)</label>
            <ImageUploadPicker
              category="vet"
              urls={documentUrls}
