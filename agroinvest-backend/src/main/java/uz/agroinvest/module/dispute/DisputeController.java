...
public class DisputeController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'SUPERADMIN', 'MODERATOR')")
    public ResponseEntity<ApiResponse<DisputeDto>> startInvestigation(
            @PathVariable UUID id,
            @AuthenticationPrincipal UserPrincipal principal
    ) {
        DisputeDto dto = disputeService.startInvestigation(id, principal);
        return ResponseEntity.ok(ApiResponse.success(dto));
    }

    @PatchMapping("/{id}/close")
    @PreAuthorize("hasAnyRole('ADMIN', 'SUPERADMIN', 'MODERATOR')")
    public ResponseEntity<ApiResponse<DisputeDto>> closeDispute(
            @PathVariable UUID id,
            @AuthenticationPrincipal UserPrincipal principal
    ) {
        DisputeDto dto = disputeService.closeDispute(id, principal);
        return ResponseEntity.ok(ApiResponse.success(dto));
    }
}
