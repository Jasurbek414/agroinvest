...
public class PaymentController {
    ...
    // Requires authentication (carved out in SecurityConfig ahead of this
    // controller's otherwise-public /api/v1/payments/** rule) since the
    // returned checkout URL embeds the caller's own user id.
    @GetMapping("/checkout-url")
    public ApiResponse<Map<String, String>> getCheckoutUrl(
            @RequestParam PaymentProvider provider,
            @RequestParam BigDecimal amount,
            @AuthenticationPrincipal UserPrincipal principal
    ) {
        String url = paymentService.buildCheckoutUrl(provider, principal.getId(), amount);
        return ApiResponse.success(Map.of("checkoutUrl", url));
    }
        ...
        try {
            switch (method) {
                ...
                case "CheckTransaction": {
                    String id = (String) params.get("id");
                    return paymentService.handlePaymeCheckTransaction(id);
                }
                default:
